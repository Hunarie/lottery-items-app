...
import { TextInput, Button, Box, Select, Accordion, AccordionItem, AccordionControl, NativeSelect, AccordionPanel } from '@mantine/core';
    ...
    return (
        <Box mx="auto" maw={800}>
            <form onSubmit={form.onSubmit((values) => console.log(values))}>
                ...
                <TextInput withAsterisk label="Item Name" value={itemForm.itemName} onChange={e => {setItemForm({...itemForm, itemName: e.target.value})}} />
                <TextInput label="Item Serial Number" value={itemForm.itemSN} onChange={e => {setItemForm({...itemForm, itemSN: e.target.value})}} />
                <TextInput label="Item Asset Tag" value={itemForm.itemAssetTag} onChange={e => {setItemForm({...itemForm, itemAssetTag: e.target.value})}} />
                <NativeSelect
                    label="Category"
                    
                    value={itemForm.itemCategory}
                    onChange={e => {setItemForm({...itemForm, itemCategory: e!.currentTarget.value})}}
                ...
                />
                <Button color='gray' mt={10} miw={800} onClick={addNewItemToArray}>+ Add New Item</Button>
                <br />
                <Button type='submit' color="gray" mt={10} mb={10} miw={800}>Create Form</Button>
